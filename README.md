...
0. There has been a deluge of new tools and technologies in the market accentuating the data engineering field and the best way to keep abreast is to pry them out and do hands-on. So, in this project there are a lot of new tools that I have used to create an effective and robust data pipeline. It goes without saying that when there are more tools involved, the integration of the tools (because of dependencies or much worse "dependency hell") becomes a big hassle but at the same time, it allows us to understand the nitty gritties of the architecture. 
...
<p align="center">
  ...
  <h6 align = "center" > Source: Maintainer </h6>
</p>
...
This command will create a structure of airflow components in your directory, that we used to manually create files earlier. The project folder structure is as shown below:
<p align="center">
...
A DAG (Directed Acyclic Graph) is a core concept and a fundamental building block. It represents a workflow or a sequence of tasks to be executed, where each task is a unit of work, and the edges between tasks define the order in which they should be executed.
    * First task:
    ...
    It is always good to test a task after creating it and so we can do that by running the following command:

    ```
    astro dev bash -> airflow tasks test <DAG name> <Task Name> <Start Date>
    ```
    This should store a file in google cloud storage bucket and mark the task as success with corresponding dag id and task id as shown below:
...
But, if we modify any check, for example, let's say we purposefully change the datatype of numeric column to string. It will throw an error.
Now to create data quality check within our DAG, for this, we will use Python external task. This task allows any python code in any external virtual environment to be executed inside the DAG. This will prevent any dependency conflict with any other open source tool you are using. To create a python virtual environment and activate it, we will modify the Dockerfile as follows: 
...
* Step 4 - Use DBT to transform the data and create fact and dimension tables.
  This is star schema data modelling in practice where we have a fact table surrounded by dimension tables. Fact table keeps all the numerical and keys of the database and dimension tables contains context and background information such as product, customer, datetime, etc.
<p align="center">
...
Now it is time to create a chain of tasks for airflow to know the direction of tasks to perform sequentially. It is done using chain method inside DAGs as shown below:
...
* Step 6 - Creating a dashboard using Metabase which is an open source data visualization and analytics platform. It will running locally on our machine and runs on localhost. To run metabase, we create a docker-compose override file as shown below:
...
This project is maintained by Jyothi Sree, a Senior Data Engineer with over 6 years of experience in architecting large-scale batch and streaming pipelines. With a focus on GCP, AWS, and Azure environments, Jyothi specializes in data modeling, Lakehouse architectures, and robust orchestration using Airflow and DBT. This implementation serves as a reference for building production-grade, quality-controlled data pipelines in modern cloud environments.
...
Acknowlegement: This project builds upon foundational data engineering patterns to demonstrate the integration of Astronomer, Soda, and DBT on Google Cloud Platform.